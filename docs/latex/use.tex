...
\begin{DoxyItemize}
\item \mbox{\hyperlink{classGroupSpecific_1_1Variables_a38586ec998bcbfdf325e6cfc6598b54b}{prime}} \+: the $p$ in $G=C_{p^n}$.
\item \mbox{\hyperlink{classGroupSpecific_1_1Variables_ac9bd6be19cc41e6877ee25a2d1c7be80}{power}} \+: the $n$ in $G=C_{p^n}$.
\item \mbox{\hyperlink{classGroupSpecific_1_1Variables_a5504789b0b60050e6ea223fdeb84874a}{reps}} \+: the number of nontrivial irreducible real representations of $G$ for our spheres.
\item \mbox{\hyperlink{classGroupSpecific_1_1Variables_a4746f16736abcf4c705dd8690ec12ca0}{sphere\+\_\+dimensions}} \+: the array consisting of the dimensions of those representations (so we must fix an order for them beforehand).
\end{DoxyItemize}\hypertarget{use_fun}{}\doxysubsection{The standard chains}\label{use_fun}

\begin{DoxyItemize}
\item There is one function that needs to be manually defined, the \mbox{\hyperlink{classGroupSpecific_1_1Function_a6d247b31d297f733e9cd6f68921a37cd}{Positive\+Chains}} computing the chains for actual (as opposed to virtual) representation spheres. The construction of these Chains is made as painless as possible, using the \mbox{\hyperlink{namespaceMackey_aa2f105c9bcbec7f5c7524027174340a0}{altmatrix}} function
...
Once Step 0 is complete, you can include {\ttfamily \mbox{\hyperlink{Additive_8h}{Additive.\+h}}} to access the methods relating to the additive structure and {\ttfamily \mbox{\hyperlink{Factorization_8h}{Factorization.\+h}}} to access the factorization methods. The multiplicative structure and Massey products are found in {\ttfamily \mbox{\hyperlink{Compute_8h}{Compute.\+h}}} but that\textquotesingle{}s already included in the other two headers.
...
The file {\ttfamily \mbox{\hyperlink{Additive_8h}{Additive.\+h}}} exposes the class \mbox{\hyperlink{classMackey_1_1AdditiveStructure}{Additive\+Structure}} that computes the homology of all spheres in a given range as \mbox{\hyperlink{namespaceMackey}{Mackey}} functors. Example\+: The code

{\ttfamily  Additive\+Structure$<$rank\+\_\+t,diff\+\_\+t$>$ A(\{-\/3,-\/4\},\{5,6\}); }

computes the homology of all spheres from $S^{-3\sigma-4\lambda}$ to $S^{5\sigma+6\lambda}$. To identify the \mbox{\hyperlink{namespaceMackey}{Mackey}} functors in the \char`\"{}universal\char`\"{} notation use
...
prints the answer in a user provided {\ttfamily stream} (eg you may pass {\ttfamily std\+::cout} to {\ttfamily print\+\_\+answer}). The answer is of the form
...
For $C_8$ there are three \mbox{\hyperlink{namespaceMackey}{Mackey}} functors not covered by our \char`\"{}universal notation\char`\"{} and are thus named \char`\"{}unknown 0,...\char`\"{}. To survey their \mbox{\hyperlink{namespaceMackey}{Mackey}} functor structure use
...
The file {\ttfamily \mbox{\hyperlink{Compute_8h}{Compute.\+h}}} exposes the method \mbox{\hyperlink{namespaceMackey_a2bd86833844ca62d76c47a54aeb0bb77}{R\+O\+Green}} that multiplies two generators in the Green functor $H_{\star}(S)$. Example\+: The code
...
where $a,b$ are generators. The answer is written as a linear combination of the generators in the latter homology group. The first argument of \mbox{\hyperlink{namespaceMackey_a2bd86833844ca62d76c47a54aeb0bb77}{R\+O\+Green}} indicates the level the generators live in (level 0=bottom, level 1= one higher etc.) so for $C_4$, level=2 is the top level. The second and third entries are the degrees of the two generators.

If the homology in these degrees is not cyclic, then the generators are not determined by their degree and need to be selected. In that case we can provide an optional final 2 {\ttfamily int} arguments in {\ttfamily \mbox{\hyperlink{namespaceMackey_a2bd86833844ca62d76c47a54aeb0bb77}{Mackey\+::\+R\+O\+Green}}} that perform the selection\+: eg $1,2$ selects the second and third generators of the noncyclic groups respectively. The default selection is $0,0$.
...
The file {\ttfamily \mbox{\hyperlink{Factorization_8h}{Factorization.\+h}}} exposes the class \mbox{\hyperlink{classMackey_1_1Factorization}{Factorization}} whose constructor creates the multiplication graph and the method \mbox{\hyperlink{classMackey_1_1Factorization_a2e135a37687fc3d69cd16a8729dd19eb}{compute\+\_\+with\+\_\+sources}} that factorizes all generators using the given sources. First construct the multiplication graph via\+:

{\ttfamily auto F= Factorization$<$rank\+\_\+t, diff\+\_\+t$>$(2,\{ -\/5,-\/5 \}, \{ 5,5 \}, \{ \{0,1,0\},\{2,2,0\},\{0,0,1\},\{2,0,1\} \}, \{ \char`\"{}asigma\char`\"{}, \char`\"{}u2sigma\char`\"{}, \char`\"{}alambda\char`\"{}, \char`\"{}ulambda\char`\"{} \});}

This will work on level $2$ (top level for $C_4$), in the range from $S^{-5\sigma-5\lambda}$ to $S^{5\sigma+5\lambda}$, by multiplying all generators of $H_{\star}S$ with the basic irreducibles $ a_{\sigma}, u_{2\sigma}, a_{\lambda}, u_{\lambda}$ of degrees $[0,1,0],[2,2,0],[0,0,1],[2,0,1]$ respectively.

Then

{\ttfamily F.\+compute\+\_\+with\+\_\+sources(\{\{0,0,0\}\}, \{\char`\"{}1\char`\"{}\});}
...
{\ttfamily std\+::cout$<$$<$ F.\+getname(\{3,1,0\}) }

To print the names of all generators use\+:

{\ttfamily std\+::cout$<$$<$ F.\+get\+\_\+generators() }

To print the multiplication graph to an {\ttfamily std\+::ofstream file} use\+:

{\ttfamily file $<$$<$ F.\+getgraph() }

or

{\ttfamily file $<$$<$ (F.\+getgraph() $<$$<$ F.\+get\+\_\+generators()) }

if you want the generator names as labels for the nodes.

A generator name being $?$ means that the generator could not be obtained by multiplying/dividing the basic irreducibles with {\ttfamily 1}. This means that additional sources may need to be added, such as through\+:

{\ttfamily F.\+compute\+\_\+with\+\_\+sources(\{\{0,0,0\},\{-\/3,0,-\/2\}\}, \{\char`\"{}1\char`\"{},\char`\"{}s\char`\"{}\});}

where now both $1$ and $s$ are used as sources.

It possible that there are enough sources yet you still get $?$; that means the basic identification was not enough to compute all products. You can then use
...
The file {\ttfamily \mbox{\hyperlink{Compute_8h}{Compute.\+h}}} finally exposes the method \mbox{\hyperlink{namespaceMackey_a9fcefe47f5a8a416b173517377a61bd0}{R\+O\+Massey}} for (triple) Massey products in the Green functor $H_{\star}(S)$. Example\+: The code
